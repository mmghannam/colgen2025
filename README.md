...
> Note: If you're not familiar with modeling languages and PySCIPOpt, you may want to take a look at the `intro` and the `modeling` folders first.
...
Next, we switch our perspective to the so-called "extended" formulation of the bin packing problem. Instead of modeling with assignments of items to bins we "extend" the formulation to look at all possible packings of items into bins. A packing is simply a subset of items that be packed into a bin (respecting its capacity). Using this concept of packings we arrive at an equivalent formulation:
